...
def ss_turnover(filename, dict1, dict2):
    ...
    for i in range(len(scoreflow)):
        if scoreflow[i]['scoreName']=="2pt Miss":
            if i+1!=len(scoreflow) and scoreflow[i+1]['period']==scoreflow[i]['period']:
                if scoreflow[i]['squadId']==team1id and scoreflow[i+1]['squadId']==team2id:
                    team1turnover+=1
                elif scoreflow[i]['squadId']==team2id and scoreflow[i+1]['squadId']==team1id:
                    team2turnover+=1
...
if __name__ == '__main__':
    team_list=['Adelaide Thunderbirds', 'Collingwood Magpies', 'GIANTS Netball', 'Melbourne Vixens', 'NSW Swifts', 'Queensland Firebirds', 'Sunshine Coast Lightning', 'West Coast Fever']
    ...
    for i in team_list:
        new_dict_for[i]=sum(ratio_for[i])/14
        new_dict_against[i]=sum(ratio_against[i])/14
    average_for=0
    for i in team_list:
    ...
    plt.plot(list(range(-1, 9)), [average_for/8]*10, color='lightgrey', linestyle='dashed', label='Average supershots scored vs turnovers from supershot attempts')
    plt.legend()
    plt.show()
